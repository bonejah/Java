...
public class MapReadWriteLockTest {
	private final static MapReadWrite mapRL = new MapReadWrite();
	...
	static class Write implements Runnable {
		...
		public void run() {
			for (int i = 0; i < 100; i++) {
				...
				try {
					mapRL.put(String.valueOf(i), String.valueOf(i));
				} finally {
	...
	static class ReadA implements Runnable {
		...
		public void run() {
			for (int i = 0; i < 100; i++) {
				...
				try {
					System.out.println(Thread.currentThread().getName() + " " + Arrays.toString(mapRL.allKeys()));
				} finally {
					rwl.readLock().unlock();
				}
			}
